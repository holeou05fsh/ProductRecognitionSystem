...
# 計算每日來客數
daily_customers = df.groupby(df['time'].dt.date).agg({'Order_ID': 'nunique'}).reset_index()
daily_customers.columns = ['time', 'customer']
# print(daily_customers)
# print(type(daily_customers))

# 計算每日收入
daily_revenue = df.groupby(df['time'].dt.date).agg({'revenue': 'sum'}).reset_index()
...
# # 配置
influx_con = _setting.set_connect()
...
data = pd.merge(data, merged_data, on='time', suffixes=('原萃綠茶', '品客洋芋片', '益生菌', '肉骨湯麵', '_customers', '_revenue'))
# print(data)
